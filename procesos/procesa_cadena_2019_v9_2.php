...
do {
    $consulta = "SELECT COUNT(*) AS numcad FROM avl_cadenas_g WHERE cad_estatus = 1";
    $query = $conn->prepare($consulta);
    $query->execute();
    $conCad = $query->fetch();
    $conCad = $conCad["numcad"];

    if ($conCad > 1000) {
        echo 'CADENAS ALTAS - '.$conCad."\n";

        $limCad = TRUE;
        do {
            // $consulta = " SELECT  * from avl_cadenas_g where cad_estatus = 1 and cad_id = (SELECT MIN(cad_id) from avl_cadenas_g WHERE cad_estatus = 1)  limit 1";
            $consulta = "SELECT * FROM (SELECT * FROM avl_cadenas_g WHERE cad_estatus = 1 ORDER BY cad_id DESC LIMIT 500) AS avl_cadenas_g  LIMIT 1";
            $query = $conn->prepare($consulta);
            $query->execute();

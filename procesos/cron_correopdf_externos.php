...
$conCir = "SELECT nombre, correos, idcliente FROM monitoreo.correosclientes WHERE status = 1 ";
$queryCir = $con13->prepare($conCir);
$queryCir->execute();
...
while ($circuito = $queryCir->fetch()) {
  ...
  // Consulta Vehiculos
  $idCli  = $circuito["idcliente"];
  $nomCir  = $circuito["nombre"];
  $corCir  = $circuito["correos"];
  $correo = explode(";", $corCir);
  $conVeh = "SELECT * FROM monitoreo.tb_vehiculos v JOIN monitoreo.unidades_renta r ON r.noeconomico = v.txt_economico_veh WHERE r.idcliente = ?";
  $queryVeh = $con13->prepare($conVeh);
  $queryVeh->bindParam(1, $idCli);
  $queryVeh->execute();
    ...
    $body = $body.'
      <tr>
        ...
        <td style="border-top:1px solid #000000;border-bottom:1px solid #000000;border-left:1px solid #000000;border-right:1px solid #000000" valign="middle" width="575" height="25" align="center">'
          .$vehiculo["txt_posicion_veh"].", ".$vehiculo["txt_upsmart_veh"].
        '</td>
      </tr>
      ';

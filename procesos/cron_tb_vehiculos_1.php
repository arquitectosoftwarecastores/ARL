...
$consulta = "SELECT * FROM ctg_vehiculos c join monitoreo.tb_vehiculos v on c.veh_nserie = v.num_serie_veh where v.txt_economico_veh < '60000';";
...
//	    echo " --- ".$registro['veh_ignicion']. " --- ";
	    if($registro['veh_ignicion']!=0){
            ...
            $num_carga = $registro['veh_velocidad'] . ";".$registro['veh_orientacion'];
			...
			txt_orientacion_veh=?, fk_clave_zon=?, num_carga = ? WHERE num_serie_veh=? ";
	    ...
	    $query2->bindParam(12, $num_carga);
            $query2->bindParam(13, $serie);
            $query2->execute();

...
do {
    ...
    $conRuta = "SELECT * 
                ...
                INNER JOIN tb_vehiculos AS tv ON trru.unidad = tv.txt_economico_veh
    ...
    while ($regRuta = $qryRuta->fetch()) {
        ...
        $conPos = "SELECT * FROM tb_posiciones 
                    WHERE num_nserie_pos = ?
                    ORDER BY fec_ultimaposicion_pos DESC
                    LIMIT 1";
        $qryPos = $conn->prepare($conPos);
        $qryPos->bindParam(1, $ser);
        $qryPos->execute();
        $regPos = $qryPos->fetch();
        ...
        # Hora Salida
        $salida = date('2020-03-27 19:00:00');
        # Hora Actual

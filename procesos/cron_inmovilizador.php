...
while (true) {
   ...
   // $consulta = " select * from monitoreo.tb_vehiculos v join monitoreo.geocercasporunidad gpu on gpu.economico = v.txt_economico_veh where v.txt_economico_veh not like '00%'";
    $consulta = " select * from monitoreo.tb_vehiculos v join monitoreo.geocercasporunidad gpu on gpu.economico = v.txt_economico_veh join monitoreo.tb_configuraciongps gps on gps.economico = v.txt_economico_veh where v.txt_economico_veh not like '00%' and gps.protocolo = 'U' and gps.linea = 1";
    ...
    while ($registro = $query->fetch()) {
        ...
        $inmovilizada = checazona($latitud1, $longitud1, -5, $conn);
	$division = checazona($latitud1, $longitud1, -6, $conn);
        echo "Economico: " . $economico1;
        echo "<br>";
        echo "Inmovilizada: " . $inmovilizada;
        echo "<br>";
        
        $inserta_geocerca = "UPDATE monitoreo.geocercasporunidad SET inmovilizador = ?, division = ?  where economico = ?";
        $query2 = $conn->prepare($inserta_geocerca);
        $query2->bindParam(3, $economico1);
	$query2->bindParam(2, $division); 
        $query2->bindParam(1, $inmovilizada);
        $query2->execute();
        $query2->closeCursor();
        
        if ($inmovilizada == 3707) {
...
// 83 Solicitar Posicionar
    ...
    $actualiza_cron_historico = "insert into monitoreo.tb_estatus_crones_historico(id_cron,fecha_registro) values (5,now())";
    $query23 = $conn->prepare($actualiza_cron_historico);
    $query23->execute();
    $query23->closeCursor();

    $actualiza_cron = "update monitoreo.tb_estatus_crones set ultimo_registro=now() where id_cron=5";
    $query24 = $conn->prepare($actualiza_cron);
    $query24->execute();
    $query24->closeCursor();	
    $query->closeCursor();
}

...
while ($registro = $query0->fetch()) {
    try {
    $query_inserta = "insert into monitoreo.lectura_tablero values (?,?,?,?,?,?)";
     $query1 = $conn->prepare($query_inserta);
     
     $query1->bindParam(1,$registro["pk_clave_veh"] );
     $query1->bindParam(2,$registro["num_serie_veh"] );
     $query1->bindParam(3,$registro["txt_odometro_veh"] );
     $query1->bindParam(4,$registro["txt_combtot_veh"] );
     $query1->bindParam(5,$registro["fec_posicion_veh"] );
     $query1->bindParam(6,$registro["txt_economico_veh"] );
     $query1->execute();

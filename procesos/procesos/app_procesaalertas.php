<?php 
	...
	while($registro1 = $query1->fetch())
		...
		{                    
	        ...
	        {
                    $id_geocerca=checazona($registro1['alerta_latitud'],$registro1['alerta_longitud'],0,$conn);
        ...
        $consultasinpos  = "select count(*) as bandera from monitoreo.vw_unidades_no_reportando_riesgo where economico = ? limit 1";  
		    ...
		    $querysinpos->execute();
                    ...
                    if(($registrosinpos["bandera"])==1){
                          $consultaultale  = "select count(*) as bandera from tb_alertas where txt_economico_veh = ? and fk_clave_tipa = 201 and fec_fecha_ale > now() - interval '30 minute' limit 1";  

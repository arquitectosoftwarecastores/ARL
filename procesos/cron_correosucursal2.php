...
while ($sucursal = $querySu->fetch()) {
  ...
  $conUni = "SELECT *, 
                ...
                FROM (
                  SELECT *,(fec_posicion_rem - '05:00:00'::interval) as fec_posicion_rem
                  FROM tb_remolques
                  WHERE fec_posicion_rem  > (now() - '05:30:00'::interval) AND
                  num_latitud_rem BETWEEN ? AND ?
  ...
  <td style="border-top:1px solid #000000;border-bottom:1px solid #000000;border-left:1px solid #000000;border-right:1px solid #000000" valign="middle" bgcolor="#0066B3" align="center"><b><font face="Liberation Serif" color="#FFFFFF">Remolque</font></b></td>
  ...
  while ($unidad = $queryUni->fetch()) {
    ...
    $uniUbi = $unidad["txt_georeferencia_mun"] . ", " . $unidad["txt_georeferencia_cas"];
    $uniFec = date("d/m/Y H:i:s", strtotime($unidad["fec_posicion_rem"]));
    echo "Unidad: ".$uniEco." Distancia: ".$uniDis." Ubicacion: ".$uniUbi." Fecha: ".$uniFec;
  }
...
//  $mail->isHTML(true);
    $mail->msgHTML($body);
  $mail->Subject = "Ubicacion de los remolques cercanos a la " . $suc . " con distancia menor a " . $rad . " KM"; // Este es el titulo del email.

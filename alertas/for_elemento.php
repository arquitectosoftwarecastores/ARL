    ...
    ?>
      <div class="row renglon">
        ...
        <div class="col-md-2">
          ...
          $fechaVer = $registro["fec_verifica_ale"];
          if (strlen($fechaVer) > 0) {
            $fechaVer = new DateTime($fechaVer, new DateTimeZone('UTC'));
            $fechaVer = $fechaVer->setTimezone(new DateTimeZone('America/Mexico_City'));
            $fechaVer = date_format($fechaVer, 'Y-m-d H:i:s');
          }
          ?>
        </div>
        ...
        <!--    <div class="col-md-1"><<!--?php echo date('d/m/Y H:i:s',strtotime($registro["fec_verifica_ale"])); ?></div>-->
        <div class="col-md-1"><?php echo $fechaVer; ?></div>
        <!-- Fin Modificaciones -->
        ...
        <form action="?seccion=alertas&amp;accion=verifica" id="form1" method="post">
          <fieldset>
            ...
            <div class="row">
              ...
              <div class="col-md-7">
                <textarea oncopy="return false" onpaste="return false" oncut="return false" class="validate[required] form-control" rows="3" id="verificaobservaciones" name="observaciones">
REMOLQUE: 
UNIDAD: 
ESTATUS: 
CLIENTE: 
...
OBSERVACION:
        ...
        <div class="row">
          <div class="col-md-12">
            <iframe id="historicoAlertas" src="../info.php" width="100%" height="200px" frameBorder="0"></iframe>
          </div>
        </div>
      </div>
      <div class="modal-footer">
...
<script>
  $('.verifica').click(function() {
    ...
    if (puesto !== '1138') {
      let unidad = '';
      if ($(this).data(unidad) !== '') {
        unidad = `&unidad=${$(this).data('unidad')}`;
      }
      console.log(unidad);
      $('#iframemapa').attr('src', `alertas/app_muestramapaGMAPS.php?economico=${$(this).data('economico')}${unidad}`)
    }
...
<div class="modal fade" id="atendida" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="staticBackdropLabel">Consulta Alerta</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="?seccion=alertas&amp;accion=reabre" id="form1" method="post">
          <fieldset>
            <fieldset>
              ...
              <div class="row mt-2 mb-1" id="rowUnidadCon">
                <div class="col-md-2 negritas derecha">
                  Unidad:
                </div>
                <div class="col-md-2" id="atendidaUnidad">
                </div>
                <div class="col-md-2 negritas derecha">
                  Oficina:
                </div>
                <div class="col-md-2" id="atendidaOficina">
                </div>
                <div class="col-md-2 negritas derecha">
                  Viaje:
                </div>
                <div class="col-md-2" id="atendidaViaje">
                </div>
              </div>
        ...
        <div class="row">
          <div class="col-md-12">
            <iframe id="historicoAlerts" src="./alertas/info.php" width="100%" height="200px" frameBorder="0"></iframe>
          </div>
        </div>
      </div>
...
<script>
  ...
  $('.atendida').click(function() {
    ...
    document.getElementById('rowUnidadCon').style.display = 'none';
    if (String($(this).data('unidad')).length > 0) {
      document.getElementById('rowUnidadCon').style.display = '';
    }
    $('#atendidaUnidad').html($(this).data('unidad'));
    $('#atendidaOficina').html($(this).data('oficina'));
    $('#atendidaViaje').html($(this).data('viaje'));

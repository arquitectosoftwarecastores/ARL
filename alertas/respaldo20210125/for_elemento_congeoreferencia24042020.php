...
/*  Almacenamos Acceso a Módulo */
$conHis = "INSERT INTO monitoreo.tb_consultas(usuario,fecha,modulo,submodulo) VALUES (?,now(),3,'ALERTAS')";
$queryHis = $conn->prepare($conHis);
$queryHis->bindParam(1, $_SESSION["usuario"]);
$queryHis->execute();
$queryHis->closeCursor();
        ...
        while ($registro = $query->fetch()) {
            ...
            <div class="row renglon">
                ...
                <?php } else { ?>
                    <div class="col-md-1">&nbsp;</div>
                <?php } ?>
                <div class="col-md-2"><?php echo date('d/m/Y H:i:s', strtotime($registro["fec_fecha_ale"])); ?></div>
                <div class="col-md-1 centrado"><?php echo $registro["txt_economico_veh"]; ?></div>
                
                <div class="col-md-1 centrado">  
                        
                    <button type="button" class="btn btn-default btn-xs"  onclick="verMapa('<?php echo $registro["txt_economico_veh"]; ?>')">
                ...
                <div class="col-md-1 centrado">
                    <?php if ($registro["num_estatus_ale"] == "0") { ?>
                        <button type="button" class="btn btn-primary btn-xs verifica" data-toggle="modal" data-target="#verifica"
                                data-observaciones="<?php echo $registro["txt_comentarios_ale"]; ?>"
                                data-fechahora="<?php echo $registro["fec_fecha_ale"] ?>"
                                data-economico="<?php echo $registro["txt_economico_veh"] ?>"
                                data-tipoalerta="<?php echo $registro["txt_nombre_tipa"] ?>"
                                data-idtipoalerta="<?php echo $registro["pk_clave_tipa"] ?>"
                                data-prioridad="<?php echo $prioridad ?>"
                                data-estatus="<?php echo $estatus . $nombre ?>"
                                data-ubicacion="<?php echo $registro["txt_ubicacion_ale"] . ", " . $registro["txt_upsmart_ale"] . "," .  $registro["num_latitud_ale"] . ", " . $registro["num_longitud_ale"] ?>"

                                >VERIFICAR</button>
                            <?php } ?>
                            <?php
                            if ($registro["num_estatus_ale"] == "1") {
                                $idGlobal = $registro["txt_economico_veh"];
                                $idGlobal2 = $registro["pk_clave_tipa"];
                                ?>
                        <button type="button" class="btn btn-default btn-xs atendida" data-toggle="modal" data-target="#atendida"
                                data-observaciones="<?php echo $registro["txt_comentarios_ale"]; ?>"
                                data-fechahora="<?php echo $registro["fec_fecha_ale"] ?>"
                                data-economico="<?php echo $registro["txt_economico_veh"] ?>"
                                data-tipoalerta="<?php echo $registro["txt_nombre_tipa"] ?>"
                                ...
                                data-prioridad="<?php echo $prioridad ?>"
                                data-estatus="<?php echo $estatus . $nombre ?>"
                                data-por="<?php echo $nombre ?>"
                                data-idG="<?php echo $registro["txt_economico_veh"] ?>"
                                data-ubicacion="<?php echo $registro["txt_ubicacion_ale"] . "<br>" . $registro["txt_upsmart_ale"] . "," .  $registro["num_latitud_ale"] . "," . $registro["num_longitud_ale"] ?>"
                                <?php
                    ...
                    where a.txt_economico_veh = ? and a.fk_clave_tipa = 201 and a.fec_fecha_ale > (now() - '240:00:00'::interval) order by fec_fecha_ale desc limit 10";

                                $queryalertas = $conn->prepare($consultaalertas);
                                $queryalertas->bindParam(1, $registro["txt_economico_veh"]);
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Verifica alertas en grupo</h4>
                </div>
...
<div id="verifica" class="modal fade" role="dialog">
    <div class="modal-dialog"  style="width: 80%;">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">&times;</button>
                <h4 class="modal-title">Verifica Alerta</h4>
            </div>
            <div class="modal-body">
			...
			<?php } ?>
                        <div class="row">
                            <input type="checkbox" id="conocida" name="conocida" value="1" /> Conocida
                            <div class="col-md-2 negritas derecha">
                                <strong>Observaciones:</strong>
                            </div>
                            <div class="col-md-7">
                                <textarea class="validate[required] form-control" rows="2" id="verificaobservaciones" name="observaciones"></textarea>
                            </div>
                            <div class="col-md-2 centrado">
                                <?php include ("general/for_filtros.php"); ?>
...
<?php
                ...
                <div class="row">
                    <div class="col-md-12">
                        <iframe id="historicoAlerts" src="../info.php?economico=95820" width="100%" height="200px" frameBorder="0"></iframe>
                    </div>
                </div>
                <br>
            </div>

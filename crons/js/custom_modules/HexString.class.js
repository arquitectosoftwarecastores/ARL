...
module.exports = class HexString {
  ...
  async validaAlertas (arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (ale.event == this.message.eventCode & ale.alert !== null) {
        ...
        await this.insertAlerta(ale.alert);
        break;
      }
    }
  ...
  /**
   * Inserta Parametros del Remolque en tb_proceso_enganche
   *
   * @param {Number} idcad Id de la cadena en avl_cadenas_g
   */
  async insertParamEnganche (idcad) {
    const inCad = `INSERT INTO monitoreo.tb_proceso_enganche
                    (txt_nserie_rem, num_ignicion_rem, num_code,
                    txt_ibutton, num_velocidad,num_latitud_pos, num_longitud_pos,estatus)

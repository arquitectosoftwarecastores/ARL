...
<?php
  ...
  $registro_tablero = $query_tablero->fetch();
   ...
   $registro8 = $query8->fetch();
            ...
            $posfecha = date('Y-m-d H:i:s', strtotime('-' . $ajustegps . ' hour', strtotime($registro8["veh_uposicion"])));
  ...
  $conInmo = "SELECT * FROM  tb_vehiculos AS tb
              LEFT JOIN unidadesrutariesgo AS urr ON urr.economico =tb.txt_economico_veh
              LEFT JOIN vehiculos_inmovilizados AS vi ON vi.noeconomico =urr.economico
              WHERE economico = ? ORDER BY fecha_entrada DESC LIMIT 1";
  $queryIn = $conn->prepare($conInmo);
  $queryIn->bindParam(1, $registro['txt_economico_veh']);
  $queryIn->execute();
  $registroIn = $queryIn->fetch();
  $estatusIn = $registroIn['estatus'];
  $automatico = $registroIn['automatico'];
  $fec_en = $registroIn['fecha_entrada'];
  $fec_sa = $registroIn['fecha_salida'];

  // Consulta Mantenimiento
  $conMan = "SELECT COUNT(*) AS mantenimiento FROM  tb_mantenimientos WHERE economico = ? AND fecha_baja IS NULL";
  $queryMan = $conn->prepare($conMan);
  $queryMan->bindParam(1, $registro['txt_economico_veh']);
  $queryMan->execute();
...
?>
    <ul class="nav nav-tabs">
      ...
      <?php
      // Oculta las demas pestañas si es usuario de WALMART 
      if ($_SESSION["nombrerol"] != "WALMART") {?>
    ...
    <div class="tab-content"  style="min-height:140px;">
      <div id="vehiculo" class="tab-pane fade in active">
        ...
        <table class="table table-striped table-bordered table-hover">
          ...
          <tr>
            <td colspan="1"><?php echo $posfecha;?></td>
            <td colspan="2">
          ...
          <?php
              // Oculta las demas pestalañas si es usuario de WALMART 
              if ($_SESSION["nombrerol"] != "WALMART") { ?>  
          <tr>
          ...
          <tr>
            <td><?php if(isset($estatusIn) AND $estatusIn == 1)echo "Si";else echo "No";?></td>
            <td>
              <?php if(isset($automatico) AND $automatico== 1)echo "Si";else echo "No"; ?>
            </td>
            <td>
                <?php if(isset($fec_en))echo $fec_en;else echo "No Aplica";?>
            </td>
            <td>
                <?php if(isset($fec_sa))echo $fec_sa;elseif(isset($fec_en)) echo "En Zona de Riesgo";else echo "No Aplica"; ?>
            </td>
          </tr>
          <?php } ?> 
          ...
          <tr>
            <td>
              ...
              // Oculta las demas pestalañas si es usuario de WALMART 
              if ($_SESSION["nombrerol"] != "WALMART") { ?>  
              <a href="http://69.172.241.230/?seccion=historico&id=<?php echo $registro["txt_economico_veh"] ?>" target="_blank">
      ...
      <div id="tablero" class="tab-pane fade in">
          <table class="table table-striped table-bordered">
            ...
            <tr>
              ...
              ?>
              <td class="centrado"><img src="imagenes/<?php echo $imagenorientacion?>"></td>
            ...
            <tr>
              ...
              <td>
                ...
                if (isset($_SESSION['mantenimiento'])) {
                 ...
                 if ($mantenimiento == 0){ ?>
                  <button type="button" class="btn btn-primary btn-xs" id="btnmant" name="btnmant" data-toggle="modal" data-target="#myModal">MANTENIMIENTO</button>
                <?php }else{ ?>
                  <a  href="?seccion=mantenimiento&accion=lista&economico=<?php  echo $registro['txt_economico_veh']; ?>&estatus=0" target="_blank">
                  <button type="button" class="btn btn-warning btn-xs" id="btnmant" name="btnmant">

<?php
  ...
  $query4->execute();  
  $consulta5  = " SELECT * FROM tb_zonas ORDER BY txt_nombre_zon ASC ";  
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <form action="?seccion=<?php echo $seccion;?>&amp;accion=agrega" id="form1" method="post">    
        ...
        <form id="AgregarPuntoIntermedio">      
        <fieldset>   
          <div class="row">            
            <div class="col-md-12">
              Punto Intermedio:<br>
              <select name="puntointermedioSelect" id="puntointermedioSelect" class="text-input text form-control" >
                <?php
                  while ($registro5 = $query5->fetch()) {
                ?>
                <option value="<?php echo $registro5['pk_clave_zon']; ?>"><?php echo $registro5['txt_nombre_zon']; ?></option>              
                <?php } ?>
              </select>
            </div>  
          </div> 
          <div class="row">   
            <div class="col-md-12">
                <p>&nbsp;</p>
                <button  id="agregapuntointermedioButton" class="btn btn-primary">AGREGAR PUNTO INTERMEDIO</button>                
            </div>  
            <br>
            <div id="ListaPuntosIntermedios"></div>
          </div>   
         </fieldset>      
        </form>          
      </div>

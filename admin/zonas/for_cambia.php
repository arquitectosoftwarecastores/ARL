<?php 
  ...
  $id=$_GET["id"];
  $consulta  = " SELECT * FROM tb_zonas WHERE pk_clave_zon=?";  
  ...
  $consulta6  = " SELECT * FROM tb_tiposdezona ";  
  $query6 = $conn->prepare($consulta6);
  $query6->execute();       
  
  $consulta7  = " SELECT * FROM tipodezona ";  
...
<div class="container">
  ...
  <div class="row"> 
    <div class="col-md-12">   
      <form action="?seccion=zonas&amp;accion=actualiza" id="form1" method="post">
          <div class="row">   
            <div class="col-md-12">
                Nombre:<input type="text" name="nombre" id="nombre" class="validate[required] text-input text form-control" size="120" maxlength="120" value="<?php echo $registro["txt_nombre_zon"]?>"  />
            </div>
          </div>
          <div class="row">
              <div class="col-md-6">
              ...
              <select name="tipo" id="tipo" class="text-input text form-control" >
                <?php
                  while ($registro6 = $query6->fetch()) {
                      if($registro['fk_clave_tipz']==$registro6['pk_clave_tipz'])
                        $seleccionado="selected";
                      else
                        $seleccionado="";                    
                ?>
                <option value="<?php echo $registro6['pk_clave_tipz']; ?>" <?php echo $seleccionado; ?>><?php echo $registro6['txt_nombre_tipz']?></option>
                <?php } ?>
              </select>
            </div>
          <div class="row">   

            ...
            if (r)
            {
                ...
                /** Indica por que metodo será enviado y preparará el comando 
                 ...
                 $.ajax({
                    ...
                    /** Valida Paro
                     * Valida si la unidad se encuentra Parada 
                     */ 
                    if (arrMsj.paro == 0) {

                      /** Verifica el tipo de protocolo y realiza el proceso de envio
                       * U (UDP): Realiza el envio por el metodo antiguo mediante PHP (con el formulario)
                       * T (TCP): Envia el comando preparado
                       */ 
                      if (arrMsj.protocolo === 'U') {

                        // Procede a enviar comando vía UDP
                        return true;
                        ...
                        // Valida si se envió el comando mediante TCP
                        if (res == '0OK') {
                          // Redirecciona a la página de comandos
                          ...
                          if (arrMsj.tip == 84) {
                            ...
                            // Procede con envio de alert
                            $.ajax({
                              type: 'POST',
                              url: './app_crea_alerta.php',
                              data: { msj: arrMsj },
                              async: false,
                              success: function(data){
                                cadJson  = JSON.parse(data);
                                ...
                                json = JSON.parse(data);
                                console.log('Error de conexion. '+ json);
                              }
                            });
                      ...
                      alert('El comando no se ha podido enviar porque es probable que la unidad se encuentre en movimiento.\nSolicita Ayuda a GPS ó Aplica el comando Obligar Paro de Motor.');
                      return false;
                   }

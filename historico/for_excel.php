...
if ($filtro == "posiciones" or $filtro == "trayectoria") {
  $consulta  = "SELECT txt_nserie_rem
                FROM tb_remolques 
  ...
  if ($fechainicial < '2018-11-05 18:26:00') {
    $tableHistorico = "tb_posiciones_historico";
  } elseif ($fechainicial < '2019-01-17 11:00:00') {
    $tableHistorico = "tb_posiciones_historico2";
  } elseif ($fechainicial < '2019-02-01 12:40:00') {
    $tableHistorico = "tb_posiciones_historico3";
  } elseif ($fechainicial < '2019-11-05 14:45:00') {
    $tableHistorico = "tb_posiciones_historico4";
  } elseif ($fechainicial < '2020-02-07 12:00:00') {
    $tableHistorico = "tb_posiciones_historico5";
  } else {
    $tableHistorico = "tb_posiciones";
  }
...
if ($_GET['filtro'] == 'eventos') {
  ...
  $consulta2  = "SELECT * 
                  ...
                  WHERE 
                    ...
                    fk_clave_tipa = pk_clave_tipa AND
                    fec_fecha_ale <= ? AND
                    fec_fecha_ale >= ?
  ...
  while ($registro2 = $query2->fetch()) {
    ...
    $fila = array(
      'latitud' => $registro2['num_latitud_ale'],
...
echo "Fecha-Hora;Posición;Velocidad;Ignición;Latitud;Longitud\n";
...
if (count($row_array) > 0) {
  ...
  foreach ($row_array as $filas) {
    ...
    echo "\"" . $filas['speed'] . " Km/hr. \"" . ";";
    if ($filas['ignicion']) echo "\"" . "Encendido" . "\"" . ";";
    else echo "\"" . "Apagado" . "\"" . ";";
    echo "\"" . $filas['latitud'] . "\"" . ";";
    echo "\"" . $filas['longitud'] . "\"" . ";";
    echo "\n";

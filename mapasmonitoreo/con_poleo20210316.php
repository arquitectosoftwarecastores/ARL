...
if ($valUni) {
	...
	$fechainicial = new DateTime($_GET["ini"]);
	//$fechainicial->add(new DateInterval('PT' . $ajusteGPS . 'H'));
	$fechainicial = date_format($fechainicial, 'Y-m-d H:i:s');
	$fechafinal = new DateTime($_GET["fin"]);
...
function consultarPosiciones ($unidad, $conn) {
	...
	while ($resPol = $qryPol->fetch()) {
		...
		$fecha = date('Y-m-d H:i:s', strtotime($resPol['fecha']));

...
if (isset($_GET['latitud']) and isset($_GET['longitud']) and isset($_GET['rad'])) {
  ...
  $consulta4 = "SELECT *, acos(sin(?)*sin(radians(num_latitud_rem)) + cos(?)*cos(radians(num_latitud_rem))*cos(radians(num_longitud_rem)-?)) * ? AS distancia  
                FROM (SELECT * FROM tb_remolques 
                      WHERE num_latitud_rem BETWEEN ? AND ? AND num_longitud_rem BETWEEN ? AND ? ) AS FirstCut 
                      WHERE acos(sin(?)*sin(radians(num_latitud_rem)) + cos(?)*cos(radians(num_latitud_rem))*cos(radians(num_longitud_rem)-?)) * ? < ? ORDER BY distancia";

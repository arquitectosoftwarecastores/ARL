...
<?php
  ...
  if(isset($_POST["radio"]))
    ...
    $radio=10;
  $consulta  = " SELECT * FROM tb_remolques ORDER BY txt_economico_rem";  
		      ...
		      <?php
		        while ($registro = $query->fetch()) {
		          if($id==$registro['txt_economico_rem'])
		            ...
		            $seleccionado="";
		      ?>
		      <option data-latitud="<?php echo $registro['num_latitud_rem'] ?>" data-longitud="<?php echo $registro['num_longitud_rem'] ?>" value="<?php echo $registro['txt_economico_rem']; ?>" <?php echo $seleccionado; ?>><?php echo $registro['txt_economico_rem']; ?></option>
		      <?php } ?>
		    </select>
	    </div> 
	    <div class="col-md-4">
  ...
  function initMap() {
    <?php 
      if($latitud==0) { 
        $zoom=5;
    ?>
        var myLatLng = {lat: <?php echo  $latitudcentro?>, lng: <?php echo  $longitudcentro?> };
    <?php } else { 
        $zoom=10;
    ?>

 ...
 -->
  ...
  <script>
    ...
    function cronStatus() {
      $.ajax({
        type: 'POST',
        url: 'general/app_crons.php',
        data: {},
        success: function(data) {
          let cronList = '';

          for (const cron of JSON.parse(data)) {
            const status = cron.estatus == 0 ? 'report' : 'correct';
            cronList += `&nbsp;&nbsp;
                      <button type="button" class="btn btn-default" data-toggle="button" aria-pressed="false">
                      <img src="assets/icons/${status}.png" height="40px" width="40px">
                      <br>
                      <b>Proceso: ${cron.id}</b>
                      </button>`;
          }
          document.getElementById('crons').innerHTML = cronList;
